 ...
 *
 * Implements automated ECC hooks for the OpenCode plugin system. Provides
 * hook-based automation across the full session, tool, file, and shell
 * lifecycle events exposed by OpenCode.
 */
...
export const ECCHooksPlugin: ECCHooksPluginFn = async ({
  ...
  return {
    ...
    "session.created": async () => {
      ...
      // Check for project-specific context files (AGENTS.md is the
      // cross-compatible standard; CLAUDE.md is supported as a fallback)
      if (hasProjectFile("AGENTS.md")) {
        log("info", "[ECC] Found AGENTS.md - loading project context")
      } else if (hasProjectFile("CLAUDE.md")) {
