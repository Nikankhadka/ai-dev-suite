...
1. **Environment variable**: `ECC_PACKAGE_MANAGER`
...
For Claude Code backward compatibility, `CLAUDE_PACKAGE_MANAGER`, `.claude/package-manager.json`, and `~/.claude/package-manager.json` are also checked as fallbacks.

## Configuration Options

### Option 1: Environment Variable
```bash
export ECC_PACKAGE_MANAGER=pnpm
```
...
echo '{"packageManager": "yarn"}' > ~/.config/opencode/package-manager.json
```
