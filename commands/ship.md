...
This command runs six stages in strict order. Each stage has a hard gate. **A failed gate after the stated max attempts means STOP and emit the report template with FAIL rows for that stage and every stage after it. Never skip a gate. Never proceed past a FAIL.**
...
Dispatch the `planner` agent with the request and the content of `AGENTS.md`.
